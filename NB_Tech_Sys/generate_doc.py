...
def main():
    ...
    doc.add_paragraph(
        'NB_Tech_Sys は、技術ドキュメント（PDF / DOCX / PPTX / TXT）をアップロードし、'
    ...
    features_intro = [
        ...
        'マルチフォーマット — PDF / Word / PowerPoint / テキストファイルに対応',
    ...
    dir_rows = [
        ['technical_support_rag.py', 'メインアプリケーション（約800行）'],
    ...
    auto_dir_rows = [
        ...
        ['models/', 'HuggingFace 埋め込みモデルのキャッシュ（約2GB）'],
    ]
    ...
    model_rows = [
        ['llama3.1:8b', 'LLM（大規模言語モデル）', '約4.9GB', '質問に対する回答を生成。Ollama 経由でダウンロード・実行。'],
        ['intfloat/multilingual-e5-large', '埋め込みモデル', '約2.2GB', 'テキストをベクトルに変換。多言語対応（日本語含む）。HuggingFace からダウンロード。'],
    ]
    ...
    doc.add_paragraph(
        ...
        'ユーザーの質問に対して類似度上位5件のチャンクを検索し、LLM へのコンテキストとして提供します。'
    )
    arch_details = [
        'インデックス方式: VectorStoreIndex（ベクトル類似度検索）',
        '検索パラメータ: similarity_top_k=5',
    ...
    doc.add_paragraph(
        'Ollama が llama3.1:8b モデルを実行し、検索結果をコンテキストとして回答を生成します。'
        '埋め込みには HuggingFace の multilingual-e5-large を使用し、多言語に対応しています。'
    )
    ai_details = [
        'LLM 温度パラメータ: 0.0（確定的な応答）',
    ...
    setup_steps = [
        ...
        ['3', 'パッケージインストール', 'requirements.txt から全依存ライブラリをインストール'],
        ['4', '埋め込みモデル DL', 'multilingual-e5-large（約2.2GB）を事前ダウンロード'],
        ['5', 'Ollama 確認', 'Ollama のインストール状況を確認、案内を表示'],
        ['6', 'LLM モデル DL', 'llama3.1:8b（約4.9GB）を Ollama 経由でダウンロード'],
    ]
    ...
    doc_steps = [
        ['1', 'ファイルアップロード', 'サイドバーから PDF / DOCX / PPTX / TXT をアップロード'],
    ...
    qa_steps = [
        ...
        ['3', 'セマンティック検索', '類似度上位5件のチャンクをインデックスから検索'],
        ['4', '回答生成', 'Ollama（llama3.1:8b）が検索結果をコンテキストとして回答を生成'],
    ...
    source_features = [
        'ファイルアップロード領域（PDF / DOCX / PPTX / TXT 対応）',
        'アップロード済みソースの一覧表示（ファイル種別アイコン付き）',
        'チェックボックスによるソース個別選択/解除',
        '選択件数バッジ（例: 「3/5 件選択中」）',
        '全ドキュメント削除ボタン（インデックスも同時削除）',
        '会話クリアボタン（ドキュメントは保持）',
    ]
    ...
    action_rows = [
        ['要約生成', 'アップロードされた全ドキュメントの要約を自動生成'],
        ['FAQ 作成', 'ドキュメントに基づく想定 Q&A を5件生成'],
        ['仕様一覧', 'ドキュメントから仕様・スペック情報を抽出してリスト化'],
    ...
    func_rows = [
        ['マルチフォーマット対応', 'PDF / DOCX / PPTX / TXT の4形式に対応'],
    ...
    req_rows = [
        ...
        ['ディスク容量', '約 7GB 以上', '埋め込みモデル 2.2GB + LLM 4.9GB + パッケージ類'],
    ...
    setup_instructions = [
        ...
        ('Step 4', 'LLM モデル（llama3.1:8b）のダウンロードが完了すると「セットアップ完了!」と表示されます。'),
    ...
    p.add_run(
        '初回セットアップには埋め込みモデル（約2.2GB）と LLM モデル（約4.9GB）の'
    ...
    qa_usage = [
        ...
        'AI がアップロードされたドキュメントの内容に基づいて回答を生成',
    ...
    doc.add_heading('13.5 ソースの絞り込み', level=2)
    doc.add_paragraph(
