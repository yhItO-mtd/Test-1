...
def main():
    ...
    model_rows = [
        ['qwen2.5:3b', 'LLM（大規模言語モデル）', '約5GB', '質問に対する回答を生成。Ollama 経由でダウンロード・実行。'],
    ...
    setup_steps = [
        ...
        ['4', '埋め込みモデル DL', 'multilingual-e5-base（約1GB）を事前ダウンロード'],
        ['5', 'Ollama 確認', 'Ollama のインストール状況を確認、案内を表示'],
        ['6', 'LLM モデル DL', 'qwen2.5:3b（約5GB）を Ollama 経由でダウンロード'],
    ]
    ...
    qa_steps = [
        ...
        ['4', '回答生成', 'Ollama（qwen2.5:3b）が検索結果をコンテキストとして回答を生成'],
    ...
    source_features = [
        'ファイルアップロード領域（PDF / DOCX / PPTX / TXT 対応、ドラッグ＆ドロップ可）',
    ...
    action_rows = [
        ['要約生成', '全ドキュメントの要約を200文字程度で生成'],
        ['FAQ 作成', 'ドキュメントに基づく想定 Q&A を生成（各回答200文字程度）'],
        ['仕様一覧', '仕様・スペック情報を200文字程度で箇条書き抽出'],
        ['トラブルシューティング', 'よくあるエラーと対処法を200文字程度で一覧生成'],
    ]
    ...
    setup_instructions = [
        ...
        ('Step 4', 'LLM モデル（qwen2.5:3b）のダウンロードが完了すると「セットアップ完了!」と表示されます。'),
    ...
    doc.add_paragraph(
        ...
        'ワンクリックで以下の操作が実行され、結果はチャットパネルに表示されます（各200文字程度）:'
    )
    quick_actions_usage = [
        '要約生成 — 全ドキュメントの内容を200文字程度で要約',
        'FAQ 作成 — よくある質問と回答を生成',
