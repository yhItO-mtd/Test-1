...
- **高品質な日本語**: Qwen2.5 (3B) による自然な日本語回答 + BGE-M3 ベクトル検索
...
| コンポーネント | メモリ使用量 |
|--------------|------------|
| Ollama + qwen2.5:3b | 約 2 GB |
| 埋め込みモデル (BGE-M3) | 約 2 GB |
...
|--------------|------|
| UI | Streamlit |
| 埋め込みモデル | BAAI/bge-m3（約 2 GB、CJK 特化） |
| ベクトルストア | LlamaIndex VectorStoreIndex（ローカル永続化） |
| LLM | Ollama - qwen2.5:3b（デフォルト、約 2 GB） |
...
1. **ソースの追加**: 左パネルからドキュメント（PDF / DOCX / PPTX / TXT / MD）をドラッグ＆ドロップでアップロード。読み込み完了後、アップローダーは自動クリアされます
2. **ソースの管理**: 「読み込み済みソース」一覧でチェックボックスから参照対象を選択。✕ ボタンで個別削除も可能
3. **質問の入力**: チャット欄に質問を入力して Enter
4. **スタジオで一括生成**: 右パネルのボタン（要約 / FAQ / 仕様一覧 / トラブル対応）をクリックすると、チャット上に回答を生成
...
├── install.bat                # Windows 用セットアップスクリプト
├── run.bat                    # Windows 用起動スクリプト
├── README.md                  # このファイル
├── models/                    # 埋め込みモデルキャッシュ（自動生成）
└── storage/                   # ベクトルインデックス & チャット履歴（自動生成）
```
