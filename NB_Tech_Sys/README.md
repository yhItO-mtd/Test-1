...
- **マルチフォーマット対応**: PDF / DOCX / PPTX / TXT をアップロード可能
- **日本語対応**: multilingual-e5-base による日本語ベクトル検索
...
| **GPU** | 不要（CPU のみで動作） | CUDA 対応 GPU があると高速化 |

### メモリ内訳（目安）

| コンポーネント | メモリ使用量 |
|--------------|------------|
| Ollama + llama3.2:1b | 約 3 GB |
...
| LLM | Ollama - llama3.2:1b（デフォルト、約 3 GB） |
...
> LLM モデル（llama3.2:1b）はアプリ初回起動時に自動でダウンロードされます。手動で事前にダウンロードする場合:
> ```bash
...
1. **ソースの追加**: 左パネルからドキュメント（PDF / DOCX / PPTX / TXT）をアップロード
2. **ソースの選択**: チェックボックスで検索対象のソースを選択
3. **質問の入力**: チャット欄に質問を入力して Enter
4. **回答の確認**: チャット内で回答と参照元を確認
